...
DEFAULT_JSONL_FILE = os.path.join("data", "news_data.jsonl")


def analyze_news_articles(
    jsonl_file: str = DEFAULT_JSONL_FILE,
    ollama_api_url: str = DEFAULT_OLLAMA_URL
) -> None:
    """
    Main function to process JSONL file with news articles through LLM analysis and sentiment scoring.
    
    Args:
        jsonl_file: Path to JSONL file containing news articles
        ollama_api_url: URL for Ollama API endpoint
    """
    process_jsonl(jsonl_file, ollama_api_url)


# Below are the original unchanged functions (now considered internal)
def get_llm_analysis(article_text: str, ollama_api_url: str) -> Optional[Dict]:
    try:
        prompt = f"""
...
def process_jsonl(jsonl_file: str, ollama_api_url: str) -> None:
